...
[^1]: Currently neither ggplot2 nor dplyr actually use these tools since I've only just figured them out. But I'll be working hard to make sure all my packages are consistent in the near future.
...
1.  For long sum expressions, `deparse()` generates a character vector with 
    length > 1:
...
*   `expr_find()` finds the underlying expression. It works similarly to 
...
A formula capture delays the evaluation of an expression so you can later evaluate it with `f_eval()`:
...
`transmogrify()` makes it easy to add new variables to a data frame:
...
If you want a `...` function that doesn't require formulas, I recommend that the SE version take a list of arguments, and the NSE version use `dots_capture()` to capture multiple arguments as a list of formulas.
